...
bool Sphere::intersect(Ray& r)
{
  ...
  if (Disc >= 0)
  {
    ...
    {
      t = - B / 2 * A;
      bool b = r.cond_set_parameter(t, this);
      ret = b;
    }
    else
    {
  ...
  if (ret) {
    Vec3f intersect = r.get_position();
    float x = intersect[0] - position[0];
    float y = intersect[1] - position[1];
    float z = intersect[2] - position[2];

    theta = acosf(z / radius);
    phi   = atan2f(y, z);

    float s = theta / M_PI;
    float t = (phi + M_PI) / (2 * M_PI);

    r.texturecoords = Vec2f(s, t);
  }
